...
只有最新的 1.4.x 版本接受安全修复。报告前请先升级到 [Releases](https://github.com/BaoWeiiii/mindbus/releases/latest) 的最新版，确认问题仍然存在。
...
- **App 自己的落盘。** 索引在 `~/Library/Application Support/MindBus/`；对话归档副本、Minds 画像、收藏在 `~/.mindbus/`（1.4.1 起目录 `700` / 文件 `600`）。削弱这些权限、或让对话内容落到同机其他账号可读的位置，算漏洞。
...
Only the latest 1.4.x release receives security fixes. Before reporting, please update to the latest version on [Releases](https://github.com/BaoWeiiii/mindbus/releases/latest) and confirm the issue is still present.

| Version | Supported |
| --- | --- |
| 1.4.x (latest) | Yes |
| < 1.4 | No — please upgrade |
...
- **The app's own files.** The index lives in `~/Library/Application Support/MindBus/`; archived conversation copies, the Minds profile, and stars live in `~/.mindbus/` (directories `700` / files `600` as of 1.4.1). Weakening those permissions, or leaking conversation content to a location readable by other accounts on the same Mac, is a vulnerability.
