...
一只收纳箱，两头服务：**你**，和**你的 AI**。它治五个痛——

### 给你的

**① 怕丢。**
Claude Code 默认 30 天静默删光全部历史，官方立场，改设置也有照删的报告。每个用满 30 天的人都会在某天发现记录没了，且无法挽回。
→ **永久留底**：每场对话在本地存一份压缩副本，工具删它的，你的还在。App 会告诉你有多少场对话「活过了那 30 天」、有多少场已经靠这份副本救了回来。

**② 难找。**
三个工具、三种格式、三个藏在 `~/` 深处的目录，想找回上周那个决定，只能一个个翻。
→ **一个窗口全收**：搜索中英文关键词直达那条消息，按工具、项目、时间随手筛，重要的标星收藏，选中内容一键接力去新对话。

**③ 无感。**
攒了几百场对话、敲了上百万字，却从来感受不到这堆东西的分量——它们只是躺在硬盘里的日志。
→ **Minds 与报告**：从你的对话里数出你的自画像——你最常让 AI 干的活、你的口头禅、反复交代过的话；随时生成统计卡片，看到自己这几个月的分量。
...
→ **内置只读 MCP**：任何接入的 AI 都能搜索这份完整历史、读你的画像——接着上次聊，而不是每次初次见面。
