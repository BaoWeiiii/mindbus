...
struct BrowserSidebarView: View {
    ...
    private func toolRowContent(_ source: ConversationSource?, active: Bool) -> some View {
        HStack(spacing: 8) {
            ...
            Spacer(minLength: 0)
            // 右侧：正在读取该工具时转小圈，读完显示它的场数——扫描进度用「哪个工具、多少场」表达
            if let s = source, progress.activeSources.contains(s) {
                ProgressView().controlSize(.mini)
            } else {
                let n = source.map { store.sourceCounts[$0] ?? 0 } ?? store.allConversations.count
                if n > 0 {
                    Text("\(n)")
                        .font(BrandFont.mono(11))
                        .foregroundStyle(active ? DSLight.gold : DSLight.t3)
    ...
    private var bottomSection: some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            // 扫描 / 归档 / 画像进行中：一行状态 + 预计剩余时间（Mail「正在检查邮件…」的位置）
            if scanStatusText != nil {
                backgroundStatusRow
                    .transition(.opacity)
            }
        ...
        .animation(.easeOut(duration: 0.3), value: scanStatusText == nil)
    }

    private var scanStatusText: String? { ScanStatus.text(progress, l10n) }

    private var backgroundStatusRow: some View {
        HStack(spacing: 8) {
        ...
        .padding(.horizontal, 14)
        .padding(.vertical, 6)
        .animation(.easeOut(duration: 0.25), value: scanStatusText)
    }
