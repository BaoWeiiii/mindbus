...
public enum VaultArchive {
    ...
    @discardableResult
    /// 归档计划：这一轮真正要压的文件与总字节——进度按字节估剩余时间用。
    /// 判定口径与 `sweep` 完全一致（源存在、过了活跃宽限、归档缺失或比源旧）。
    public static func plan(paths: [String], root: URL = defaultRoot, now: Date = Date())
        ...
        for path in paths {
            ...
            if let archivedAt = archiveModificationDate(sourcePath: path, root: root), archivedAt >= mtime {
                continue
            }
            out.append(path)
            bytes += Int64((attrs[.size] as? Int) ?? 0)
        }
        return (out, bytes)
    }

    public static func sweep(paths: [String], root: URL = defaultRoot, now: Date = Date(),
                             onEach: ((Int64) -> Void)? = nil) -> Int {
        let fm = FileManager.default
        var count = 0
        for path in paths {
            let attrs = try? fm.attributesOfItem(atPath: path)
            let size = Int64((attrs?[.size] as? Int) ?? 0)
            defer { onEach?(size) }   // 跳过的也算「处理过」：进度按计划范围走
