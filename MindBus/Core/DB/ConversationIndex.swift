...
public final class ConversationIndex: @unchecked Sendable {
    ...
    CREATE TABLE IF NOT EXISTS minds_contagion (
        path TEXT PRIMARY KEY,
        mtime REAL NOT NULL,
        lex_key INTEGER NOT NULL,
    ...
    // MARK: - Minds 画像缓存（传染词的单场摘要）

    /// 画像段按会话缓存的中间结果：path → (源文件 mtime, 词表指纹, 摘要 JSON)。
    /// 此前只在进程内存里——每次启动 App，第一轮扫描都要把全库源文件重新解析一遍
    /// 算画像（本机 18GB：约 70 秒满一核、1.9GB 峰值），用户的感受就是「一打开系统
    /// 卡几秒」。落盘后只有源文件变过、或词表变过的会话才重算。
    public struct MindsCacheRow {
        public let path: String
        public let mtime: Double
        public let lexKey: Int64
        public let summary: Data
        public init(path: String, mtime: Double, lexKey: Int64, summary: Data) {
            self.path = path; self.mtime = mtime; self.lexKey = lexKey; self.summary = summary
        }
    }

    public func loadMindsCache() -> [MindsCacheRow] {
        var out: [MindsCacheRow] = []
        try? queue.sync {
            try db.query("SELECT path, mtime, lex_key, summary FROM minds_contagion;", row: { st in
                let n = Int(sqlite3_column_bytes(st, 3))
                let blob = sqlite3_column_blob(st, 3).map { Data(bytes: $0, count: n) } ?? Data()
                out.append(MindsCacheRow(path: SQLiteDB.text(st, 0), mtime: sqlite3_column_double(st, 1),
                                         lexKey: sqlite3_column_int64(st, 2), summary: blob))
            })
        }
        return out
    }

    /// 写入这一轮重算过的会话，并删掉库里已不存在的会话的缓存。
    public func saveMindsCache(upsert rows: [MindsCacheRow], keeping livePaths: Set<String>) {
        try? queue.sync {
            try db.transaction {
                var stale: [String] = []
                try db.query("SELECT path FROM minds_contagion;", row: { st in
                    let p = SQLiteDB.text(st, 0)
                    if !livePaths.contains(p) { stale.append(p) }
                })
                for p in stale {
                    try db.run("DELETE FROM minds_contagion WHERE path = ?;") { SQLiteDB.bindText($0, 1, p) }
                }
                for r in rows {
                    try db.run("""
                        INSERT INTO minds_contagion (path, mtime, lex_key, summary) VALUES (?, ?, ?, ?)
                        ON CONFLICT(path) DO UPDATE SET mtime = excluded.mtime, lex_key = excluded.lex_key,
                                                        summary = excluded.summary;
                        """) { st in
                        SQLiteDB.bindText(st, 1, r.path)
                        sqlite3_bind_double(st, 2, r.mtime)
                        sqlite3_bind_int64(st, 3, r.lexKey)
                        r.summary.withUnsafeBytes { buf in
                            _ = sqlite3_bind_blob(st, 4, buf.baseAddress, Int32(buf.count), SQLiteDB.transient)
                        }
                    }
                }
            }
    ...
    public func forgottenRelated(to conversationID: String,
        ...
        // 同分必须有确定的先后：tf 是字典，遍历顺序随进程的哈希种子变，只按分数排的话
        // 平分的词（回退切出的二字组全是 tf=1、idf 相同）谁进前六完全看运气——
        // 2026-09-08 CI 就这样随机零召回。同分时长词优先（更具体），再按字面序。
        .sorted { a, b in
    ...
    /// 十分钟内还在被写的会话不进指纹（口径同 VaultArchive.activeGrace）：活跃会话每轮
    /// 扫描都在变，指纹跟着变就是每轮都整份重建画像——2026-09-07 实测一次 19 秒满一核，
    /// 用户每聊几句就来一回，启动时也必来一回。会话凉下来那一刻才进指纹、触发一次重建。
    /// 段 / 语料 / 里程碑 / 实体都随会话派生，不再单独计数；mcp_refs 来自引用日志、
    /// 独立于会话，保留。
    public func changeFingerprint(now: Date = Date()) -> String {
        var parts: [String] = []
        let cutoff = now.timeIntervalSince1970 - VaultArchive.activeGrace
        try? queue.sync {
            try db.query("SELECT COUNT(*), COALESCE(MAX(mtime), 0), COALESCE(SUM(message_count), 0) FROM conversations WHERE mtime <= ?;",
