...
public final class ConversationStore: ObservableObject {
    ...
    /// 列表刷新后是否自动选中最新一条。首启欢迎页展示期间由界面置 false。
    public var autoSelectsFirst = true

    /// 各来源的会话数（侧栏工具行右侧的数字）
    @Published public private(set) var sourceCounts: [ConversationSource: Int] = [:]
    ...
    public init(indexPath: String = ConversationStore.defaultIndexPath()) {
        ...
        // 来源开关变了：侧栏立刻跟上，并重扫一轮（关掉的来源在收尾被移出索引）
        NotificationCenter.default.addObserver(forName: SourcePreferences.changed, object: nil, queue: .main) { [weak self] _ in
            Task { @MainActor [weak self] in
                guard let self else { return }
                self.refreshAvailableSources()
                self.applyFilters()
                self.refresh(minInterval: 0)
            }
        }
    }
    ...
    public func refreshAvailableSources() {
        Task.detached(priority: .utility) { [weak self] in
            ...
            await MainActor.run { [weak self] in
                guard let self else { return }
                let avail = Self.browsableSources.filter(detected.contains).filter(SourcePreferences.isEnabled)
    ...
    /// - Parameter lightweight: 扫描期间的中途刷新——只更新列表与标签色，跳过引用徽章（SQL）
    ///   与救回集合（每行两次 stat）；这两样在扫完的最终 setAll 里补。首建库每秒一次的
    ///   中途刷新若带上它们，几千行 × stat 全落在主线程，界面直接卡死（2026-09-07 实测）。
    public func setAll(_ lites: [ConversationLite], lightweight: Bool = false) {
        self.allConversations = lites
        self.sourceCounts = lites.reduce(into: [:]) { $0[$1.source, default: 0] += 1 }
        if !lightweight {
    ...
    public func loadAsync() {
        ...
        Task.detached(priority: .userInitiated) {
            ...
            // 2. 后台增量扫描；扫描期间每秒把已入库的行刷进列表——对话随扫描逐条出现，
            //    首建库的用户不用盯着空列表等整轮扫完。
            let finished = ScanFlag()
            let scan = Task.detached(priority: .utility) {
                await LoaderRuntime.indexAllSources(into: index)
                finished.set()
            }
            var lastCount = existing.count
            while !finished.value {
                try? await Task.sleep(nanoseconds: 2_000_000_000)
                guard !finished.value else { break }
                let rows = index.allMetadata()
                guard rows.count != lastCount else { continue }   // 没新行就别动列表
                lastCount = rows.count
                await MainActor.run {
                    self.setAll(rows, lightweight: true)
    ...
    /// 扫描完成标记：给 loadAsync 的刷新循环用（跨线程读写，锁保护）。
    ...
    private func applyFilters() {
        ...
        let disabled = SourcePreferences.disabledSources()   // 读一次，别在几千行的循环里逐行读 UserDefaults
        filteredConversations = allConversations.filter { lite in
            ...
            if let f = focus {
                ...
                guard Self.browsableSources.contains(lite.source),
                      !disabled.contains(lite.source) else { return false }
            }
        ...
        // 只在「从没选过」时补选最新一条（首启欢迎页期间关掉：自动选中会立刻解析最新
        // 那场——可能是几百 MB 的 rollout——并把欢迎页顶掉；用户点了哪条再开哪条）。
        //
        ...
        if selectedConversationId == nil, autoSelectsFirst {
