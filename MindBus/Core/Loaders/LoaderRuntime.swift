...
public enum LoaderRuntime {
    ...
    public static func indexAllSources(into index: ConversationIndex) async {
        ...
        let task: Task<Void, Never> = {
            ...
            let t = Task.detached(priority: .utility) {
                // 用户关掉的来源不扫（SourcePreferences，向导来源页 / 设置页开关）
                let enabled = SourcePreferences.isEnabled
                let sources: [ConversationSource] = [.claudeCode, .codex, .claudeAgent, .browser]
                ScanProgress.shared.begin(expectedSources: sources.filter(enabled).count)
                await withTaskGroup(of: Void.self) { g in
                    if enabled(.claudeCode) { g.addTask { await onUtilityQueue { ClaudeCodeLoader.loadAll(into: index) } } }
                    if enabled(.codex) { g.addTask { await onUtilityQueue { CodexLoader.loadAll(into: index) } } }
                    if enabled(.claudeAgent) { g.addTask { await onUtilityQueue { ClaudeAgentLoader.loadAll(into: index) } } }
                    if enabled(.browser) { g.addTask { await onUtilityQueue { BrowserVaultLoader.loadAll(into: index) } } }
                }
                await onUtilityQueue { pruneMissing(from: index) }
                await onUtilityQueue { pruneDisabledSources(from: index) }
                // 归档扫尾：把索引里该归档而未归档的源文件补齐。
                // 必须在 pruneMissing 之后 —— prune 先清掉政策排除与真正无救的行，
                // 剩下的才是值得占归档空间的。
                await onUtilityQueue {
                    // 先算这一轮真正要压多少字节，剩余时间才估得准；sweep 只跑计划内的文件
                    let plan = VaultArchive.plan(paths: archiveScope(of: index))
                    ScanProgress.shared.beginArchiving(totalBytes: plan.bytes, files: plan.paths.count)
                    let n = VaultArchive.sweep(paths: plan.paths,
                                               onEach: { ScanProgress.shared.archiveFileDone(bytes: $0) })
                    if n > 0 { NSLog("[vault] archived %d new conversation file(s)", n) }
                    ScanProgress.shared.beginFinishing()
                }
                // 个人词表：语料涨了就重学（——语料越多词表越准）。放在 sweep
                // 之后纯粹是顺序上的收尾位置，两者无数据依赖。
                // 收尾各步计时进 scan.log：2026-09-07 实测扫描三段只要 2 秒、但启动后满一核 20 秒，
                // 差的都在这几步里，没有计时根本不知道该优化谁。
                let steps = StepTimer()
                await onUtilityQueue {
                    steps.measure("lexicon") { _ = index.rebuildLexiconIfNeeded() }
                }
                // MCP 引用回流（第5步/§2.2）：汇入 memory_open 记的 jsonl 到
                // 聚合表 mcp_refs。放在词表重建之后同样是顺序上的收尾位置，
                // 与上面词表重建互不依赖，全量重算本身也是幂等的。
                await onUtilityQueue {
                    steps.measure("refs") { index.ingestRefLog(from: MCPRefLog.defaultLogURL) }
                }
                // 文件夹改名追踪:百级目录各一次 stat,发现「同 inode 换路径」即自动
                // 写别名——历史会话的标签下轮刷新即合流到新名。人工别名优先(tracker 内保证)。
                await onUtilityQueue {
                    steps.measure("folders") {
                ...
                await onUtilityQueue {
                    steps.measure("minds") { MindsBuilder.build(from: index) }
                }
                // ~/.mindbus 权限兜底：目录 700 / 文件 600（见 MindBusHome）
                await onUtilityQueue { steps.measure("perms") { MindBusHome.tightenPermissions() } }
                ScanLog.append("[scan] finish: " + steps.line)
                ScanProgress.shared.finish()
    ...
    /// 用户关掉的来源：已入库的行移除，下轮扫描也不再进来。归档副本不动。
    /// 只处理三个本地工具来源；browser 行的 file_path 是相对伪路径，不参与。
    static func pruneDisabledSources(from index: ConversationIndex) {
        let disabled = SourcePreferences.disabledSources()
            .intersection([.claudeCode, .claudeAgent, .codex])
        guard !disabled.isEmpty else { return }
        let paths = index.allMetadata()
            .filter { disabled.contains($0.source) }
            .map(\.fileURL.path)
        guard !paths.isEmpty else { return }
        index.prune(missingPaths: paths)
    }
    ...
    public static func indexRows(
        ...
        ScanProgress.shared.sourceStarted(source)
        defer { ScanProgress.shared.sourceFinished(source) }
        ...
        // 进度条按字节走：待处理总量先报上去，每个文件解析完再报完成量
        ScanProgress.shared.addPlanned(source: source, bytes: Int64(staleSizes.values.reduce(0, +)), files: stale.count)
        ...
        let lock = NSLock()
        var batch: [(lite: ConversationLite, segments: [Segmenter.Segment], mtime: Double, entityText: String, userText: String, lastRole: String, harvest: MindsMilestones.Harvest)] = []
        ...
        for item in stale {
            ...
            group.enter()
            DispatchQueue.global(qos: .utility).async {
                var row: (lite: ConversationLite, segments: [Segmenter.Segment], mtime: Double, entityText: String, userText: String, lastRole: String, harvest: MindsMilestones.Harvest)? = nil
                autoreleasepool {
                    ...
                    if var r = makeRow(item.url) {
                        if TombstoneStore.shared.isBuriedConversation(id: r.lite.id) {
                            ...
                            row = (r.lite, r.segments, item.mtime, r.entityText, r.userText, r.lastRole, r.harvest)
                        }
                    }
                }
                parseGate.signal()   // parse 完即放槽（upsert 不占解析并发）
                ScanProgress.shared.fileDone(bytes: Int64(staleSizes[item.url.path] ?? 0))
                if isBig {
                ...
                var pending: [(lite: ConversationLite, segments: [Segmenter.Segment], mtime: Double, entityText: String, userText: String, lastRole: String, harvest: MindsMilestones.Harvest)]? = nil
                lock.lock()
...
/// 收尾各步的耗时（只记秒数，不记内容），写进 scan.log。
final class StepTimer {
    private var out: [String] = []
    private let lock = NSLock()
    func measure(_ label: String, _ work: () -> Void) {
        let t = Date(); work()
        let s = String(format: "%@ %.1fs", label, Date().timeIntervalSince(t))
        lock.lock(); out.append(s); lock.unlock()
    }
    var line: String { lock.lock(); defer { lock.unlock() }; return out.joined(separator: " · ") }
}
