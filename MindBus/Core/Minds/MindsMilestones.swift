...
///    且分不开「自己写的」和「贴进来的」。
///
/// 10. **URL 引用**（你反复贴的链接 = 反复回去看的资料）
///     加「亲手贴过 + 跨 ≥1 天」两道闸后只剩 5 条,全是配置示例和报错残渣;
///     真有价值的（langgraph / OWASP）都是 AI 说的,分不清你有没有真看过。
///
/// 11. **强调标记**（你用【】「」** 圈出的内容,显式动作 + 书写系统级结构）
///     判据完全符合准则,但产出与短语层高度重合（「人工录入」本就在榜）,
///     其余是粘贴文档的结构残渣（Step 1: / Merge / Read）。
///     教训:符合准则只保证「能识别」,不保证「有增量」——新信号还要过
///     「现有层没覆盖」这一关。
///
