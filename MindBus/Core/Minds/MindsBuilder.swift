...
public enum MindsBuilder {
    ...
    public static func build(from index: ConversationIndex, to url: URL = defaultMindsURL) {
        ...
        let now = Date()
        // 各节耗时记进 scan.log（只有秒数）：整份重建 19 秒满一核，不计时不知道该优化谁
        var marks: [(String, TimeInterval)] = []
        var lastMark = now
        func mark(_ label: String) {
            let n = Date(); marks.append((label, n.timeIntervalSince(lastMark))); lastMark = n
        }
        ...
        mark("vocabStats+technicalTerms")
        ...
        let top = marks.sorted { $0.1 > $1.1 }.prefix(6)
            .map { String(format: "%@ %.1fs", $0.0, $0.1) }.joined(separator: " · ")
        ScanLog.append(String(format: "[minds] build %.1fs — ", Date().timeIntervalSince(now)) + top)
    ...
    /// ContagionSummary 的落盘形状（元组不能 Codable）。键名压到一个字母：
    /// 一场几百个词、全库几千场，这份 JSON 的体量主要就是键名和句子。
    struct ContagionSummaryDTO: Codable {
        struct Hit: Codable { var t: Double; var s: String }
        var a: [String: Hit]
        var u: [String: Hit]
        var w: [String]
        var n: [String]

        init(_ s: ContagionSummary) {
            a = s.aiFirst.mapValues { Hit(t: $0.at.timeIntervalSince1970, s: $0.sentence) }
            u = s.userFirst.mapValues { Hit(t: $0.at.timeIntervalSince1970, s: $0.sentence) }
            w = Array(s.userWords)
            n = Array(s.seen)
        }

        var summary: ContagionSummary {
            var s = ContagionSummary()
            s.aiFirst = a.mapValues { (Date(timeIntervalSince1970: $0.t), $0.s) }
            s.userFirst = u.mapValues { (Date(timeIntervalSince1970: $0.t), $0.s) }
            s.userWords = Set(w)
            s.seen = Set(n)
            return s
        }
    ...
    /// 词表指纹。必须跨进程稳定——Swift 的 Hasher 每次启动换随机种子，拿它当落盘
    /// 缓存的键，重启后永远不命中。FNV-1a，词按字典序、以换行分隔。
    static func lexiconKey(_ words: some Sequence<String>) -> Int64 {
        var h: UInt64 = 0xcbf29ce484222325
        for w in words.sorted() {
            for b in w.utf8 { h ^= UInt64(b); h = h &* 0x100000001b3 }
            h ^= 0x0A; h = h &* 0x100000001b3
        }
    ...
    /// 顺带淘汰已删除的会话。进程启动后第一次用时从索引库的 minds_contagion 表回灌
    /// （`store`），重算过的条目写回去——见 ConversationIndex.loadMindsCache。
    private static var contagionCache: [String: (mtime: Double, lexKey: Int64, summary: ContagionSummary)] = [:]
    private static var contagionCacheLoaded = false
    private static let contagionCacheLock = NSLock()

    /// 还在被写的会话（十分钟内改过）沿用上一份摘要：活跃会话每轮扫描都在变，每轮
    /// 整份重解析一个几百 MB 的 rollout 就是每次刷新都满一核二十秒；画像是统计聚合，
    /// 晚十分钟没有区别，等它凉了再重算一次。口径同归档的 activeGrace。
    static let hotFileGrace: TimeInterval = VaultArchive.activeGrace

    static func contagionAcrossLibrary(metadata: [ConversationLite], lexicon: Set<String>,
                                       limit: Int, functionWords: Set<String>,
                                       store: ConversationIndex? = nil) -> [Contagion] {
        let words = lexicon.filter { $0.count >= 3 }
        var canonical: [String: String] = [:]
        for w in words { canonical[w.lowercased()] = w }
        let lexKey = lexiconKey(words)

        contagionCacheLock.lock()
        if !contagionCacheLoaded, let store {
            let decoder = JSONDecoder()
            for row in store.loadMindsCache() {
                guard let dto = try? decoder.decode(ContagionSummaryDTO.self, from: row.summary) else { continue }
                contagionCache[row.path] = (row.mtime, row.lexKey, dto.summary)
            }
            contagionCacheLoaded = true
        }
        let cache = contagionCache
        contagionCacheLock.unlock()
        var fresh: [String: (mtime: Double, lexKey: Int64, summary: ContagionSummary)] = [:]
        var computed: [ConversationIndex.MindsCacheRow] = []   // 这一轮真正重算过的，写回库
        let encoder = JSONEncoder()
        var summaries: [(summary: ContagionSummary, cwd: String)] = []
        summaries.reserveCapacity(metadata.count)
        // 画像段的进度按字节：先 stat 一遍拿 (mtime, size)，命中缓存的会话瞬间记完成
        let fm = FileManager.default
        let stats: [(lite: ConversationLite, mtime: Double, size: Int64)] = metadata.map { lite in
            let attrs = try? fm.attributesOfItem(atPath: lite.fileURL.path)
            let mtime = (attrs?[.modificationDate] as? Date)?.timeIntervalSince1970 ?? -1
            let size = Int64((attrs?[.size] as? Int) ?? 0)
            return (lite, mtime, size)
        }
        ScanProgress.shared.beginProfiling(totalBytes: stats.reduce(0) { $0 + $1.size })
        defer { ScanProgress.shared.beginFinishing() }
        let now = Date().timeIntervalSince1970
        var hits = 0, hotReused = 0, parsedBytes: Int64 = 0
        let started = Date()
        defer {
            ScanLog.append(String(format: "[minds] profile: %d conversations, %d cached, %d hot reused, %d parsed (%.0f MB) in %.1fs",
                                  stats.count, hits, hotReused, stats.count - hits - hotReused,
                                  Double(parsedBytes) / 1e6, Date().timeIntervalSince(started)))
        }
        for (lite, mtime, size) in stats {
            let path = lite.fileURL.path
            defer { ScanProgress.shared.profileFileDone(bytes: size) }
            if let c = cache[path], c.lexKey == lexKey {
                let hot = now - mtime < hotFileGrace
                if c.mtime == mtime || hot {
                    if c.mtime == mtime { hits += 1 } else { hotReused += 1 }
            ...
            // 大文件整份解析完就把 malloc 空闲池还给系统：否则池水位停在最大那场的
            // 高点，整段画像期间 RSS 挂在 1.6–1.9GB（2026-09-07 冷建库实测）
            if size >= 32 * 1024 * 1024 { malloc_zone_pressure_relief(nil, 0) }
            guard let summary else { continue }
            summaries.append((summary, lite.cwd))
            fresh[path] = (mtime, lexKey, summary)
            if let blob = try? encoder.encode(ContagionSummaryDTO(summary)) {
                computed.append(.init(path: path, mtime: mtime, lexKey: lexKey, summary: blob))
            }
        }
        contagionCacheLock.lock()
        contagionCache = fresh
        contagionCacheLock.unlock()
        if let store, !computed.isEmpty || fresh.count != cache.count {
            store.saveMindsCache(upsert: computed, keeping: Set(fresh.keys))
        }
        return mergeContagion(summaries, limit: limit, functionWords: functionWords)
    }
